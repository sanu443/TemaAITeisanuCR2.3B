...
public class LC_Solver {
    public static void main(String[] args) {
        for (int test = 1 ; test <= 7; test++) {
            ...
            LC_Solver solver = new LC_Solver(dist, nrCities, start);
    ...
    public LC_Solver () {}
    public LC_Solver (int _dist[][], int _nrCities, int _start) {
    ...
    private class Path implements Comparable<Path> {
        ...
        public int compareTo(Path path) {
           if (this.cost == path.cost) return 0;
           return (this.cost < path.cost) ? -1 : 1;
        }
    }
}
